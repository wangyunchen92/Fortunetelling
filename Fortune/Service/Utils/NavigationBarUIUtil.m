...
+ (UIView *)createNavRightButton:(NSString *)text imageName:(NSString *)imageName
{
    ...
    UIButton *btn = [UIUtil createButton:rtBtn title:text titleColor:kColorHWBlack bgImage:IMAGE_NAME(imageName) target:nil action:nil];
    UIButton *bgBtn = [UIUtil createButton:rtBgBtn title:text titleColor:kColorHWBlack bgImage:IMAGE_NAME(imageName) target:self action:nil];
    [bgBtn addSubview:btn];
                  ...
                  action:(SEL)action
{
    ...
    if (title) {
        [view addSubview:[NBUtil createNavTitle:NBFrameTitle title:title]];
    }
